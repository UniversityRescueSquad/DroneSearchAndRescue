...
class ObjectDetector:
    ...
    def fine_tune(
        ...
        Args:
            ...
            patience (int, optional): The patience value for early stopping. Defaults to 3.
        ...
        for epoch in range(num_epochs):
            ...
            train_loss = self._train_one_epoch(
                train_loader, optimizer, loss, matcher, epoch
            )
    ...
    def _train_one_epoch(
        ...
        for i, (images, targets) in enumerate(data_loader):
            ...
            if i % print_freq == 0:
                logging.info(
                    f"Epoch: [{epoch + 1}][{i}/{len(data_loader)}] Loss: {losses.item():.4f}"
                )
    ...
    def _evaluate(
        ...
        with torch.no_grad():
            for image, targets in data_loader:
                ...
                targets = [{t: targets[t].to(self.device) for t in targets}]
                ...
                total_predictions += sum([len(t["labels"]) for t in targets])
