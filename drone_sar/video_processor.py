...
class VideoProcessor:
    ...
    def save_output_video(self, output_file_path: str) -> None:
        ...
        self.output_video = cv2.VideoWriter(
            output_file_path, fourcc, fps, (width, height)
        )
    ...
    def _draw_boxes(self, frame: np.ndarray, detections: dict) -> np.ndarray:
        for score, label, box in zip(
            ...
            cv2.line(
                frame,
                (x1 + padding, y1 + padding),
                (x1 + padding + edge_length, y1 + padding),
                color,
                edge_thickness,
            )
            cv2.line(
                frame,
                (x1 + padding, y1 + padding),
                (x1 + padding, y1 + padding + edge_length),
                color,
                edge_thickness,
            )
            # Top right corner
            cv2.line(
                ...
                (x2 - padding - edge_length, y1 + padding),
                color,
                edge_thickness,
            )
            cv2.line(
                frame,
                (x2 - padding, y1 + padding),
                (x2 - padding, y1 + padding + edge_length),
                color,
                edge_thickness,
            )
            # Bottom left corner
            cv2.line(
                ...
                (x1 + padding + edge_length, y2 - padding),
                color,
                edge_thickness,
            )
            cv2.line(
                frame,
                (x1 + padding, y2 - padding),
                (x1 + padding, y2 - padding - edge_length),
                color,
                edge_thickness,
            )
            # Bottom right corner
            cv2.line(
                frame,
                (x2 - padding, y2 - padding),
                (x2 - padding - edge_length, y2 - padding),
                color,
                edge_thickness,
            )
            cv2.line(
                frame,
                (x2 - padding, y2 - padding),
                (x2 - padding, y2 - padding - edge_length),
                color,
                edge_thickness,
            )
